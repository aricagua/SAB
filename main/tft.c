...
void TFTspiWrite(uint8_t spidata)
{
    ...
    {
        if (t & 0x80)
            gpio_set_level(TFT_SDO, 1);
        else
            gpio_set_level(TFT_SDO, 0);
        gpio_set_level(TFT_SCK, 1);
        t = t << 1;
...
void TFT_Initialize(uint8_t SCK, uint8_t SDO, uint8_t RST, uint8_t CS, uint8_t DC)
{
    // TFT_Initialize(16, 32, 5, 4, 17);
    ...
    gpio_reset_pin(TFT_SCK);
    gpio_set_direction(TFT_SCK, GPIO_MODE_OUTPUT);
    gpio_set_level(TFT_SCK, 0);

    gpio_reset_pin(TFT_SDO);
    gpio_set_direction(TFT_SDO, GPIO_MODE_OUTPUT);
    gpio_set_level(TFT_SDO, 0);
...
// Draw fills a rectangle starting from coordinates (x,y) with width of w and height of h.
...
// Used internally by fill circle fillRoundRect
...
// Draws a fill circle of coordinates (x0,y0), and r radius
...
// Draws a rectangle with rounded edges. h: height, w:width, r: radius of the rounded edges.
...
// Draws a filled rectangle with rounded edges. h: height, w:width, r: radius of the rounded edges.
...
// Draws a filled triangle of coordinates (x0,y0), (x1,y1) and (x2,y2).
...
void TFTdrawCharV(uint8_t x, uint8_t y, uint8_t c, uint16_t color, uint16_t bg, uint8_t size)
{
    ...
    for (i = 0; i < _TFTCurrentFontWidth; i++)
    {
        uint8_t line = Font_Default[(c - ASCIIOffset) * _TFTCurrentFontWidth + i];
        for (j = 6; j >= 0; j--, line >>= 1)
        {
            if (line & 0x01)
            {
                if (size == 1)
                    TFTdrawPixel(y + j, x + i, color);
                else
                    TFTfillRect(y + (j * size), x + (i * size), size, size, color);
            }
            else if (bg != color)
            {
                if (size == 1)
                    TFTdrawPixel(y + j, x + i, bg);
                else
                    TFTfillRect(y + j * size, x + i * size, size, size, bg);
            }
        }
    }
}
...
// Writes text (*text) on the TFT at coordinates (x, y). size: text size.
...
void TFTchangeMode(ST7735_modes_e mode)
{
    ...
    case ST7735_modes_Normal:
        if (_TFTcurrentMode == ST7735_modes_DisplayOff)
        { // was in off display?
            TFTwriteCommand(ST7735_DISPON);
        }
        if (_TFTcurrentMode == ST7735_modes_Idle)
        { // was in idle?
            TFTwriteCommand(ST7735_IDLEOF);
        }
        if (_TFTcurrentMode == ST7735_modes_Sleep)
        { // was in sleep?
            TFTwriteCommand(ST7735_SLPOUT);
            vTaskDelay(pdMS_TO_TICKS(120));
        }
        if (_TFTcurrentMode == ST7735_modes_Invert)
        { // was inverted?
            _TFTcurrentMode = 0;
            TFTwriteCommand(ST7735_INVOFF);
        }
...
void TFTFontNum(ST7735_FontType_e FontNumber)
{
    ...
    {
    case 1:
        _TFTCurrentFontWidth = (setfontwidth = TFTFont_width_5);
        break; // Norm default 5 by 8
    case 2:
        _TFTCurrentFontWidth = (setfontwidth = TFTFont_width_7);
        break; // Thick 7 by 8 (NO LOWERCASE LETTERS)
    case 3:
        _TFTCurrentFontWidth = (setfontwidth = TFTFont_width_4);
        break; // Seven segment 4 by 8
    case 4:
        _TFTCurrentFontWidth = (setfontwidth = TFTFont_width_8);
        break; // Wide  8 by 8 (NO LOWERCASE LETTERS)
    case 5:
        _TFTCurrentFontWidth = (setfontwidth = TFTFont_width_3);
        break; // Tiny 3 by 8
    }
}
